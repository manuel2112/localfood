...
class Cliente extends CI_Controller {
	...
	public function __construct()
	{
        ...
        $this->session_id 	= $this->session->userdata('clienteappchan');
        $this->plan_id 		= $this->session->userdata('clienteplanappchan');
	}
	...
	public function index()
	{
		...
		$data['tiposdecomida'] 		= $this->tipo_comida_model->getTipoComidaEmpresa($this->session_id);
		$data['tiposdenegocio'] 	= $this->tipo_negocio_model->getTipoNegocioEmpresa($this->session_id);
		$data['tiposdepago'] 		= $this->tipo_pago_model->getTipoPagoEmpresa($this->session_id);
		$data['horario']			= $this->empresa_horario_model->getEmpresaHorarioRow($this->session_id);
	...
	{
		$idEmpresa		= $this->input->post('idEditCliente');
		...
		//UPDATE EMPRESA
		$this->empresa_model->updateEmpresaCliente($idEmpresa, $txtDireccion, $txtUrl, $txtFono, $txtDescripcion);
		$this->estadistica_back_model->insertEstadisticaBack($this->session_id,2,fechaNow());//INGRESO EMPRESA

		//VALIDAR IMAGEN
		if( !empty($_FILES["EditFotoLogoCliente"]["tmp_name"]) ){

			list($ancho,$alto) = getimagesize($_FILES["EditFotoLogoCliente"]["tmp_name"]);
			...
			if( $_FILES["EditFotoLogoCliente"]["type"] == "image/jpeg" ){
				$aleatorio	= generaRandom();
				$ruta		= $directorio."/".$aleatorio.".jpg";
				
				$origen		= imagecreatefromjpeg($_FILES["EditFotoLogoCliente"]["tmp_name"]);
			...
			if( $_FILES["EditFotoLogoCliente"]["type"] == "image/png" ){
				$aleatorio	= generaRandom();
				$ruta		= $directorio."/".$aleatorio.".png";
				
				$origen		= imagecreatefrompng($_FILES["EditFotoLogoCliente"]["tmp_name"]);
				$destino	= imagecreatetruecolor($nuevoAncho,$nuevoAlto);
			...
			$this->estadistica_back_model->insertEstadisticaBack($this->session_id,7,fechaNow());//INGRESO EMPRESA
		}
		
		$success = 'Cliente editado exitosamente exitosamente.';
