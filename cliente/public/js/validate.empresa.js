...
$(document).ready(function(){
	...
	$(".nuevaFotoEmpresa,.EditFotoEmpresa,.nuevaFotoPromo").change(function(){
...
$(document).ready(function(){
	...
	$("#cmbCiudad,#txtEmpresa,#txtFono,#txtDescripcion,.chktipocomida,.chktiponegocio,.chktipopago").change(function(){
		...
		//VALIDAR TIPO COMIDA		
		var boolTipoComida = false;
		if (($("input[name*='chktipocomida']:checked").length) > 0) { boolTipoComida = true; }
		//VALIDAR TIPO NEGOCIO		
		var boolTipoNegocio = false;
		if (($("input[name*='chktiponegocio']:checked").length) > 0) { boolTipoNegocio = true; }
		//VALIDAR TIPO PAGO		
		var boolTipoPago = false;
		if (($("input[name*='chktipopago']:checked").length) > 0) { boolTipoPago = true; }
		
		//COMPROBAR TODOS LOS CAMPOS
		if( boolCiudad && boolEmpresa && boolFono && boolDescripcion && boolTipoComida && boolTipoNegocio && boolTipoPago ){
	...
	$("#empresa").on("click", ".btnGetVerEmpresa", function(){
		...
		$.ajax({
			...
			success: function(respuesta){
				...
				$.each(respuesta.tiposdecomida , function( index, value ) {
					txtTiposComida += "-" + value.TIPO_COMIDA_NOMBRE + "<br>";
				});				
				$("#verTiposComida").html(txtTiposComida);				
				
				$.each(respuesta.tiposdenegocio , function( index, value ) {
					txtTiposNegocio += "-" + value.TIPO_NEGOCIO_NOMBRE + "<br>";
				});				
				$("#verTiposNegocio").html(txtTiposNegocio);
				
				$.each(respuesta.tiposdepago , function( index, value ) {
					txtTiposPago += "-" + value.TIPO_PAGO_NOMBRE + "<br>";
				});
				$("#verTiposPago").html(txtTiposPago);
	...
	$("#empresa").on("click", ".btnGetEditarEmpresa", function(){
		...
		$.ajax({
			...
			success: function(respuesta){
				...
				//TIPOS DE COMIDA
				$.each(respuesta.tiposdecomidaall , function( index, value ) {
					
					var checkedTipoComida = "";
					$.each(respuesta.tiposdecomida , function( indexch, valuech ) {
						if( valuech.TIPO_COMIDA_ID === value.TIPO_COMIDA_ID ){
							checkedTipoComida = "checked";
							return;
						}
					});
						   
					txtTiposComida += '<div class="form-check"><label class="form-check-label"><input type="checkbox" class="form-check-input chktipocomida" name="chktipocomida[]" value="'+ value.TIPO_COMIDA_ID +'" '+ checkedTipoComida +'> '+ value.TIPO_COMIDA_NOMBRE +'</label></div>';
				});				
				$("#editarTiposComida").html(txtTiposComida);

				//TIPOS DE NEGOCIO
				$.each(respuesta.tiposdenegocioall , function( index, value ) {
					
					var checkedTipoNegocio = "";
					$.each(respuesta.tiposdenegocio , function( indexch, valuech ) {
						if( valuech.TIPO_NEGOCIO_ID === value.TIPO_NEGOCIO_ID ){
							checkedTipoNegocio = "checked";
							return;
						}
					});
						   
					txtTiposNegocio += '<div class="form-check"><label class="form-check-label"><input type="checkbox" class="form-check-input chktiponegocio" name="chktiponegocio[]" value="'+ value.TIPO_NEGOCIO_ID +'" '+ checkedTipoNegocio +'> '+ value.TIPO_NEGOCIO_NOMBRE +'</label></div>';
				});				
				$("#editarTiposNegocio").html(txtTiposNegocio);

				//TIPOS DE PAGO
				$.each(respuesta.tiposdepagoall , function( index, value ) {
					
					var checkedTipoPago = "";
					$.each(respuesta.tiposdepago , function( indexch, valuech ) {
						if( valuech.TIPO_PAGO_ID === value.TIPO_PAGO_ID ){
							checkedTipoPago = "checked";
							return;
						}
					});
						   
					txtTiposPago += '<div class="form-check"><label class="form-check-label"><input type="checkbox" class="form-check-input chktipopago" name="chktipopago[]" value="'+ value.TIPO_PAGO_ID +'" '+ checkedTipoPago +'> '+ value.TIPO_PAGO_NOMBRE +'</label></div>';
				});				
				$("#editarTiposPago").html(txtTiposPago);
